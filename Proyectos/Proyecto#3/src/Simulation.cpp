...
void Simulation::init(const char* title, int xpos, int ypos, int width, int height, bool fullscreen)
{
	...
	tilemap = new GameObject("assets/Nivel.png", renderer);
	character = new GameObject("assets/char2.png", renderer);
...
void Simulation::update(float x, float y, float rot, float dt)
{
	...
	character->update((x - 16) * xScale, (y - 16) * yScale, 32 * xScale, 32 * yScale);
}
